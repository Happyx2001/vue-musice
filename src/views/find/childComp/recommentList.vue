    ...
    <row-scroll :probeType='3' ref="rowScroll">
      <div class="content">
        <div
          ...
          :key="index"
          @click="ToPageSongList(item.creativeId)"
        >
          <div class="item-img">
            <img :src="item.resources[0].uiElement.image.imageUrl">
            <span class="count">{{filterCount(item.resources[0].resourceExtInfo.playCount)}}</span>
          </div>
          <div class="descript">
            <span>{{item.resources[0].uiElement.mainTitle.title}}</span>
          </div>
        </div>
      </div>
    </row-scroll>
